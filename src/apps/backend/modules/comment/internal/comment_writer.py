...
class CommentWriter:
    ...
    def create_comment(*, params: CreateCommentParams) -> Comment:
        comment_bson = CommentModel(
            author_id=params.author_id, text=params.text, task_id=params.task_id
        ).to_bson()
        ...
        return CommentUtil.convert_task_bson_to_task(created_comment_bson)
    ...
    def update_comment(*, params: UpdateCommentParams) -> Comment:
        updated_comment_bson = CommentRepository.collection().find_one_and_update(
            {"_id": ObjectId(params.comment_id), "author_id": params.author_id, "task_id": params.task_id},
    ...
    def delete_comment(*, params: DeleteCommentParams) -> CommentDeletionResult:
        comment = CommentReader.get_comment(params=GetCommentParams(author_id=params.author_id, task_id=params.task_id, comment_id=params.comment_id))
        ...
        if updated_comment_bson is None:
            raise CommentNotFoundError(task_id=params.task_id)
