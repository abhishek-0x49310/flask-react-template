...
class BaseTestComment(unittest.TestCase):
    ...
    DEFAULT_COMMENT_TEXT = "This is a test comment text"
    ...
    def create_test_comment(self, author_id: str, task_id: str, text: str = None) -> Comment:
        return CommentService.create_comment(
            params=CreateCommentParams(
                author_id=author_id,
                task_id=task_id,
                text=text or self.DEFAULT_COMMENT_TEXT
            )
        )
    
    def create_multiple_test_comments(self, author_id: str, task_id: str, count: int) -> list[Task]:
        comments = []
        for i in range(count):
            comment = self.create_test_comment(author_id=author_id, task_id=task_id, text=f"Comment {i+1}")
    ...
    def make_authenticated_request(
        self, method: str, account_id: str, token: str, task_id: str, comment_id: str = None, data: dict = None, query_params: str = ""
    ):
        if comment_id:
    ...
    def assert_comment_response(self, response_json: dict, expected_comment: Comment = None, **expected_fields):
        ...
        assert response_json.get("author_id") is not None
        assert response_json.get("task_id") is not None

        if expected_comment:
            assert response_json.get("id") == expected_comment.id
            assert response_json.get("author_id") == expected_comment.author_id
            assert response_json.get("text") == expected_comment.text
